...
export class SummarizeCommand implements ISlashCommand {
	...
	public async executor(context: SlashCommandContext, read: IRead, modify: IModify, http: IHttp): Promise<void> {
		...
		const notPosted = await this.getNotPosted(room, read, context.getSender(), threadId);
	...
	private async summarizeMessages(
		...
		if (!response.content) {
			await this.notifyMessage(room, read, user, 'Something is wrong with AI. Please try again later');
	...
	private async getThreadMessages(room: IRoom, read: IRead, user: IUser, threadId: string) {
	...
	private async getNotPosted(room: IRoom, read: IRead, user: IUser, threadId: string) {
		const threadReader = read.getThreadReader();
		const thread = await threadReader.getThreadById(threadId);
	...
	private async notifyMessage(room: IRoom, read: IRead, user: IUser, message: string, threadId?: string): Promise<void> {
